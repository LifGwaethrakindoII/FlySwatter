...
namespace Voidless
{
	public static class VTransform
	{
        ...
        /// <summary>Creates children and assigns local transform properties (relative to this Transform that will act as parent).</summary>
        /// <param name="_transform">Transform that will create the child.</param>
        /// <param name="_localPosition">Local Position relative to this Transform.</param>
        /// <param name="_localRotation">Local Rotation relative to this Transform.</param>
        /// <param name="_localScale">Local Scale relative to this Transform.</param>
        /// <param name="name">Name to assign to this child (empty by default).</param>
        /// <returns>Created child.</returns>
        public static Transform CreateChildWithLocalProperties(this Transform _transform, Vector3 _localPosition, Quaternion _localRotation, Vector3 _localScale, string name = "")
        {
            if (string.IsNullOrEmpty(name)) name = "Child_" + _transform.gameObject.name;

            Transform child = new GameObject(name).transform;

            child.SetParent(_transform);
            child.localPosition = _localPosition;
        ...
        /// <summary>Creates children with default local transform properties (relative to this Transform that will act as parent).</summary>
        /// <param name="_transform">Transform that will create the child.</param>
        /// <param name="name">Name to assign to this child (empty by default).</param>
        /// <returns>Created child.</returns>
        public static Transform CreateChildWithLocalProperties(this Transform _transform, string name = "")
        {
            return _transform.CreateChildWithLocalProperties(Vector3.zero, Quaternion.identity, Vector3.one, name);
        }

        public static Transform GetChildWithName(this Transform transform, string name, bool createIfNonExistent = true)
        {
