...
namespace Voidless
{
	...
	public static class VString
	{
		...
		/// <summary>Evaluates that object is either null or not.</summary>
		/// <param name="_obj">Object reference to evaluate</param>
		/// <returns>NOT_NULL if different than null, NULL otherwise</returns>
		public static string EvaluateForNullReference(this object _obj)
		{
			return _obj != null ? "NOT_NULL" : "NULL";
		}
		...
		public static string CollectionToString<T>(this ICollection<T> _collection, bool _appendLine = true)
		{
			...
			if(_appendLine) builder.AppendLine(">:");
			else builder.Append(">: { ");
			...
			while(iterator.MoveNext())
			{
				...
				if(!_appendLine && i < _collection.Count - 1) builder.Append(",");
				i++;
			}

			if(!_appendLine) builder.Append(" }");
			return builder.ToString();
		}
		...
		/// <summary>Routine that returns a typed text over time.</summary>
		/// <param name="_text">Text to type.</param>
		/// <param name="_typingCooldown">Typing Cooldown.</param>
		public static IEnumerator<string> TypeTextRoutine(this string _text, float _typingCooldown)
		{
			if(string.IsNullOrEmpty(_text)) yield return null;
			...
			for(int i = 0; i < length - 1; i++)
			{
				...
				while(wait.MoveNext()) yield return builder.ToString();
				wait.Reset();
			}

			builder.Append(_text[length - 1]);
