...
namespace Voidless
{
	...
	#region NO_BOSS
        ...
        /// <summary>Returns a rotation that would make the Y-Axis rotate towards the direction [Quaternion.LookRotation makes the Z-Axis rotate towards the direction].</summary>
        /// <param name="d">Look Direction.</param>
        /// <param name="u">Up vector reference.</param>
        public static Quaternion LookRotationUp(Vector3 d, Vector3 u)
        {
            // Ensure the vectors are normalized.
            u.Normalize();
            d.Normalize();

            // Calculate the rotation using the cross product and the dot product.
            Vector3 rotationAxis = Vector3.Cross(u, d);
            float rotationAngle = Mathf.Acos(Vector3.Dot(u, d));
        ...
        /// <summary>Interpolates a rotation A towards a rotation B, around a given axis.</summary>
        /// <param name="a">Rotation A.</param>
        /// <param name="b">Rotation B.</param>
        /// <param name="axis">Rotation Axis.</param>
        /// <param name="t">Normalized time t [internally clamped].</param>
        public static Quaternion LerpAroundAxis(Quaternion a, Quaternion b, Vector3 axis, float t)
        {
            // Ensure the rotation axis is normalized.
            axis.Normalize();

            // Calculate the rotation angles for start and end vectors.
            float startAngle = Mathf.Acos(Quaternion.Dot(a, b));
            float endAngle = Mathf.Acos(Quaternion.Dot(b, a));

            // Create quaternions for the rotations around the axis.
            Quaternion startRotation = Quaternion.AngleAxis(startAngle * Mathf.Rad2Deg, axis);
            Quaternion endRotation = Quaternion.AngleAxis(endAngle * Mathf.Rad2Deg, axis);

            return Quaternion.Lerp(startRotation, endRotation, t);
        }
    }
}