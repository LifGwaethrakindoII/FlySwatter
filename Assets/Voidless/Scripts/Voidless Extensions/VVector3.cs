...
#region DistanceFunctions:
	...
	#region CollectionMethods:
		...
		*/
        ...
        /// <summary>Quadratic Bezier Function.</summary>
        ...
        /// <returns>Quadratic Bezier Curve.</returns>
        ...
        /// <returns>Vector's Average of its 3 components.</returns>
        public static float Average(this Vector3 v)
        {
            return (v.x + v.y + v.z) / 3.0f;
        }

        /// <summary>Interpolates a vector A towards a vector B, around a given axis.</summary>
        /// <param name="a">Vector A.</param>
        /// <param name="b">Vector B.</param>
        /// <param name="axis">Rotation Axis.</param>
        /// <param name="t">Normalized time t [internally clamped].</param>
        public static Vector3 LerpAroundAxis(Vector3 a, Vector3 b, Vector3 axis, float t)
        {
            // Ensure the rotation axis is normalized.
            axis.Normalize();

            // Calculate the rotation angles for start and end vectors.
            float startAngle = Mathf.Acos(Vector3.Dot(a, b));
            float endAngle = Mathf.Acos(Vector3.Dot(b, a));
            ...
            // Interpolate the rotations using Quaternion.Lerp.
            Quaternion interpolatedRotation = Quaternion.Lerp(startRotation, endRotation, t);

            return interpolatedRotation * a;
        }
        ...
        {
            return new Vector3(
                Mathf.Abs(b.x) > 0.0f ? 0.0f : a.x,
                Mathf.Abs(b.y) > 0.0f ? 0.0f : a.y,
                Mathf.Abs(b.z) > 0.0f ? 0.0f : a.z
            );
        }
    }
}